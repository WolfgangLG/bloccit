...
RSpec.describe SponsoredPostsController, type: :controller do
  ...
  let(:my_sponsored_post) { my_topic.sponsored_posts.create!(valid_attributes) }
  ...
  describe "POST #create" do
    it "increases the number of SponsoredPost by 1" do
      expect{post :create, topic_id: my_topic.id, sponsored_post: valid_attributes}.to change(SponsoredPost,:count).by(1)
    end

    it "assigns the new sponsored_post to @sponsored_post" do
      post :create, topic_id: my_topic.id, sponsored_post: valid_attributes
